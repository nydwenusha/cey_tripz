...
{
    private function bookingValidator(Request $request, bool $isUpdate = false): ValidationValidator
    {
        $rules = [
            'customer_name' => 'required|string|max:255',
            'customer_email' => 'required|string|email|max:255',
            'customer_phone' => ($isUpdate ? 'nullable' : 'required') . '|string|max:20',
        ...
        if ($isUpdate) {
            $rules['status'] = 'required|string|in:pending,confirmed,cancelled,completed';
        }

        $validator = Validator::make($request->all(), $rules);

        $validator->after(function ($validator) use ($request, $isUpdate) {
            if ($validator->errors()->has('vehicle_type') || $validator->errors()->has('passengers')) {
                return;
            }

            $vehicleQuery = Vehicle::query()->where('name', trim((string) $request->vehicle_type));

            if (!$isUpdate) {
                $vehicleQuery->where('status', 'active');
            }
            ...
            if ((int) $request->passengers > (int) $vehicle->capacity) {
                $validator->errors()->add(
                    'passengers',
                    "This vehicle can carry a maximum of {$vehicle->capacity} passengers."
                );
            }
        });
    ...
    {
        $validator = $this->bookingValidator($request);
    ...
    {
        $validator = $this->bookingValidator($request, true);
